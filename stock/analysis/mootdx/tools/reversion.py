...
from analysis.mootdx.utils.factor import fq_factor
...
def factor_reversion(symbol: str, method: str = 'qfq', raw: pd.DataFrame = None) -> pd.DataFrame:
    ...
    if not factor.empty:
        ...
        if method == 'qfq':
            data.factor.bfill(axis="index", inplace=True)
        else:
            data.factor.ffill(axis="index", inplace=True)
        # data.factor = data.factor.fillna(method=('ffill', 'bfill')[method == 'qfq'], axis=0)
        data.factor = data.factor.fillna(value=1.0, axis=0)
