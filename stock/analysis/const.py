...
def latest_trading_day(t:int = 0) -> datetime.date:

    if t == 0:
        dt_date = dt_now
    else:
        dt_date =  dt_now - datetime.timedelta(days=t)
    ...
    if df_trade.at[str_dt_date, "is_open"] == 1:
        ...
        str_dt_out = df_trade.at[str_dt_date, "pretrade_date"]
...
filename_signal = os.path.join(path_check, f"signal_{str_filename_trading}.xlsx")
filename_data_csv = os.path.join(path_check, f"trader_{str_filename_trading}.csv")
