...
# 腾讯分钟线
def get_price_min_tx(code, count=10, frequency="1d"):  # 分钟线获取
...
def get_price(code, end_date="", count=10, frequency="1d"):  # 对外暴露只有唯一函数，这样对用户才是最友好的
    ...
    if frequency in ["1m", "5m", "15m", "30m", "60m"]:  # 分钟线 ,1m只有腾讯接口  5分钟5m   60分钟60m
        if frequency in "1m":
            return get_price_min_tx(xcode, count=count, frequency=frequency)
        try:
            return get_price_sina(
                xcode, end_date=end_date, count=count, frequency=frequency
            )  # 主力
        except:
            return get_price_min_tx(xcode, count=count, frequency=frequency)  # 备用
